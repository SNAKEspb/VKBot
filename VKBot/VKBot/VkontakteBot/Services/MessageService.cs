...
namespace VKBot.VkontakteBot.Services
{
    public class MessageService
    {
        ...
        static List<MessagePatternHandler> patternHandlers = new List<MessagePatternHandler>()
        {
            ...
            new MessagePatternHandlers.TextAny(@".*", 10, 2, new List<string>(){ DataService.vityaId}),
            new MessagePatternHandlers.TextBatman(@"бетмен", 10, 0),
            new MessagePatternHandlers.TextBot(@"(^|\.|\,|\s)бот(\w{0,2})?", 10, 0),
            new MessagePatternHandlers.TextHttp(@"^http", 10, 0),
            new MessagePatternHandlers.TextJoke(@"((?:заеб|залуп|говн|плох|хуй|хуе).*шутк[a|и]?)|((?:^)?шутк[a|и]?.*(?:заеб|залуп|говн|плох|хуй|хуе|заеб))|(шутит|шутишь|шутканул|пошутил)", 10, 0),
            new MessagePatternHandlers.TextRespect(@"уважение|увожение", 10, 0),
            new MessagePatternHandlers.UserCommandMeme(@"(?:^!|(?:говорит|сказал):?\s(?:что)?)\s*(.*)",10, 1),
